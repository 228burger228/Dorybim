// ─── GALLERY DATA ───
// Замени url на реальные изображения Дори
const works = [
  { cat: 'chars',     emoji: '🧑‍🎨', label: 'Персонаж', title: 'Девушка в цветах',     color: '#FFD8C0' },
  { cat: 'cafe',      emoji: '☕',    label: 'Кафе',      title: 'Уютное кафе утром',    color: '#E8D5C0' },
  ...
  { cat: 'chars',     emoji: '✨',    label: 'Персонаж',  title: 'Магический портрет',   color: '#E8D8F0' },
...
function renderGallery(cat) {
  ...
  filtered.forEach((w, i) => {
    ...
    // Make first item span 2 cols if showing all or enough items
    if (i === 0 && filtered.length >= 3) {
      item.style.gridColumn = 'span 2';
      item.style.aspectRatio = '2/1';
    }
    item.innerHTML = `
      <div class="gallery-placeholder" style="background: linear-gradient(135deg, ${w.color}, #FAF6F0);">
        <span>${w.emoji}</span>
        <p>${w.label}</p>
      </div>
      <div class="gallery-overlay">
        <div class="gallery-overlay-text">${w.title}</div>
      </div>
    `;
    ...
    item.addEventListener('click', () => {
      // When real images are added, open lightbox here
      // openLightbox(w.url);
    });
